    ...
    const fetchPendingSubjectResultSheets = async (id) => {
        try {
            ...
            if(lectureType === "subject-lecture"){
               response = await axios.get(`${host}/api/results/lec1/${id}/PENDING`);
            }else{
              console.log("second marker")
                response  = await axios.get(`${host}/api/results/lec2/${id}/PENDING`);
            }
    ...
    return (
      <div>
        <div className="flex flex-col items-center">
          ...
          <div className="px-[4.5%] mt-10 w-full flex items-center justify-start">
            <div 
                onClick={()=>{setLectureType("subject-lecture")}}
                className={`px-2 py-2 w-40 flex items-center justify-center ${lectureType==="subject-lecture"?"bg-blue-50":"bg-transparent"} text-[0.95rem] border-[1px]
                    border-blue-500 cursor-pointer rounded-l-xl` }
            >
                As Subject Lecture
            </div>
            <div 
                onClick={()=>{setLectureType("second-marker")}}
                className={`px-2 py-2 w-40 flex items-center justify-center ${lectureType==="second-marker"?"bg-blue-50":"bg-transparent"} text-[0.95rem] border-[1px]
          ...
          </div>
          {
            ...
            (
                <ResultSheetList userType="lecture-1" resultSheets={resultSheets} />
            )
            :
            (
                <ResultSheetList userType="lecture-2" resultSheets={resultSheets} />
            )
          }
        </div>
      </div>
