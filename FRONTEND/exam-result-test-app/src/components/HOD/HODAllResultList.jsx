...
import { setResultsSheets } from '../../store/reducers/HODNavigationSlice';

function HODAllResultList() {
    ...
    const [originalResultSheets, setOriginalResultSheets] = useState([]); // Keep original sheets
    const [filteredResultSheets, setFilteredResultSheets] = useState([]); // For displaying results
    ...
    const fetchPendingSubjectResultSheets = async (id) => {
        try {
            const response = await axios.get(`${host}/api/results/hod/getNotApprove/${id}`);
    ...
    if (error) return <p className='p-[5%] text-[1rem]'><br/>{error}</p>;
    ...
    return (
        <div>
            <div className='flex flex-col items-center'>
                ...
                <ResultSheetList userType="HOD" resultSheets={filteredResultSheets} /> {/* Use filtered sheets for display */}
            </div>
        </div>
    );
}

export default HODAllResultList;
